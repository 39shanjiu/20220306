...
//这里偏移量起始是对齐数但也不完全是......。
//结构体的内存对齐
//偏移量--结构体中的元素在内存中的存储的起始位置由偏移量的倍数决定。
//偏移量的大小由元素大小和编辑器的默认偏移量决定(比较取小)。
//vs默认偏移量为8.
//结构体一开始的偏移量为0；
//struct stu
//{
//	这是一个错误示范，因为一个元素的对齐数大小只看数据类型就行，是不是数组都没有关系，所以这里完全错误
//	char name[20];//第一个元素开始存储，存储完后偏移量为20.
//	char tele[11];//偏移量为11，vs为8，以八为的倍数为起始位置。
//	所以从24开始（21~23的空间浪费）存储完后偏移量为35
//	char sex[10];//偏移量为10比8大，从40开始，存储完后为50；
//	int age;//int类型为4，比8小，从52开始，存储完后为56.
//	全部元素存储完后，根据所有元素中数据最大的最后划分存储空间大小
//	这里最大为20，所以最后为20的倍数。即60；
//
//};//正确的是前三个对齐数都是1，全都直接存储就行到int时偏移量是41，从4的倍数开始，即44存储完后为为48
//是成员变量的最大对齐数的倍数（不算编辑器的对齐数），所以最后为48。
//int main()
//{
//	struct stu s1 = { 0 };
...
//	int c : 10;//10比特位
//	int d : 30;//30个比特位
//};//47个比特位，按理说是6个字节，但却是8个字节。
////一开始，根据int类型分配4个字节，存储到d时，空间不够，就开辟新的空间（之前剩余的浪费）
////还是int类型，4个字节。所以是8个字节
//
//int main()
//{
//	struct S s;
